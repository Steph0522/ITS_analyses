gg_ordiplot3 <- function(ord, groups, groups2, scaling = 1, conf=NULL, show.groups="all", spiders = FALSE, pt.size = 3, plot=TRUE) {
  library(ggordiplots)
  library(vegan)
  ...
  df_ord <- ord$Vectors #scaling=scaling, choices=choices)
  names<- df_ord$SampleID
  ...
  xlab <- paste("PC1", round(axis.labels[1]*100, digits = 2), "%")
  ylab <- paste("PC2", round(axis.labels[2]*100, digits = 2), "%")
  plt <- ggplot2::ggplot() +
