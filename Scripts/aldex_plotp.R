...
#' @export
aldex.plotp<-function (x, ..., type = c("MW", "MA", "volcano", "volcano.var"), xlab = NULL, ylab = NULL,
    ...
    rare.cex = 0.2, main=NULL)
{
    ...
    if (test == "welch") {
        ...
        if ( length(x$we.ep) > 0 ){ 
        #warning('using we.ep') 
        	p.add <- min(x$we.ep[x$we.ep > 0])/10
        	
        	called <- x$we.ep <= cutoff.pval
       		all.p <- x$we.ep + p.add
        } 
    }
    else if (test == "wilcox") {
        ...
        if (cutoff.effect <= 0.49)
            stop("Please set cutoff to at least 0.5")
        called <- abs(x$effect) >= cutoff.effect & x$we.ep <= cutoff.pval
    }
    if (type == "MW") {
        if (is.null(xlab))
  ...
  if (type == "volcano") {
        if (is.null(ylab))
            ylab <- expression("-1 * Median Log"[10]~" p value")
        if (is.null(xlab))
