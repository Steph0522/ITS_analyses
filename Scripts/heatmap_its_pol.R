...
table_genus<- tab_taxa %>%  separate(
    ...
    dplyr::mutate(Genus = stringr::str_trim(Genus, side = "both")) %>% mutate_if(
      ...
      Genus) %>% summarise_if(is.numeric, sum) %>% 
...
heats<-ComplexHeatmap::Heatmap(
 ...
 #height = nrow(merge_data)*unit(2.4, "mm"),
  ...
  left_annotation = c(anntro, annguild, annphyl),
...
heatm
ggsave('Plot/heatmap_all_bypol_noroder_phyl_15.png', width = 11, height = 4, dpi = 300, plot =heatm)
