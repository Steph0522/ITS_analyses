...
labels <- all.lm %>%  t()  %>% 
  ...
  mutate(`Poligono       ` = case_when(
    `Poligono       ` < 0.001 ~ "< 0.001",
    TRUE ~ paste0("= ", `Poligono       `))) %>%
  mutate(`Season         ` = case_when(
    `Season         ` < 0.001 ~ "< 0.001",
    TRUE ~ paste0("= ", `Season         `))) %>%
  mutate(`Season:Poligono` = case_when(
    `Season:Poligono` < 0.001 ~ "< 0.001",
    TRUE ~ paste0("= ", signif(`Season:Poligono`, digits = 3)))) %>%
  mutate(labels = paste0(letter, "       Season ", `Season         `, 
                         ", Polygon ", `Poligono       `, 
                         ", Season:Polygon ", `Season:Poligono`))
