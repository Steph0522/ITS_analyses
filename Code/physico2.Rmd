...
pvals <- melted_df %>%
  dplyr::group_by(conservation2) %>%
  dplyr::summarise(p = summary(aov(value ~ Poligono))[[1]][["Pr(>F)"]][1]) %>%
  dplyr::mutate(label = ifelse(p < 0.001, 
                             "p < 0.001", 
                             paste0("p = ", format(round(p, 3), nsmall = 3)))) %>%
  dplyr::mutate(Poligono = "P3")

pvals <- pvals %>%
  dplyr::left_join(
    melted_df %>% 
      dplyr::group_by(conservation2) %>% 
      dplyr::summarise(value = max(value, na.rm = TRUE) * 1.05),
    by = "conservation2")
...
#  stat_compare_means(method = "anova", label = "p.format", label.x = 2.5)
  geom_text(data = pvals, aes(x = Poligono, y = value, label = label), 
