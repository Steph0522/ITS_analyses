...
table_otus<- read_qza("../Data/table_SMOQ.qza")$data
...
q0_vs_depth_fltr <- ggscatter(q_data , 
  ...
  theme(legend.title = element_blank(), legend.position = "none")+ labs(y=expression(paste(italic("q"), "=0", " (number of total features/OTUs)")))
...
q1_vs_depth_fltr <- ggscatter(q_data, 
  ...
  theme(legend.title = element_blank(), legend.position = "none")+ labs(y=expression(paste(italic("q"), "=1", " (number of frequent features/OTUs)")))
...
q2_vs_depth_fltr <- ggscatter(q_data, 
  ...
  theme(legend.title = element_blank(), legend.position = "none")+ labs(y=expression(paste(italic("q"), "=2", " (number of dominant features/OTUs)")))
...
labels<- labels  %>% mutate(val=1600)
 ...
 a <- qs %>%   mutate(
    orders = case_when(
      order == "q0" ~ "italic('q') ~ '=0'",
      order == "q1" ~ "italic('q') ~ '=1'",
      order == "q2" ~ "italic('q') ~ '=2'" )) %>%filter(
        order=="q0") %>% 
  ggbarplot(x = "Season", y = "val", fill="Season", facet.by = "Poligono", add=c("mean_se", "jitter"))+
  facet_grid2(vars(orders), vars(Poligono), scales = "free", labeller = label_parsed, strip = strip)+
...
b <- qs %>%   mutate(
    orders = case_when(
      ...
      order == "q2" ~ "italic('q') ~ '=2'" )) %>%filter(
        order=="q1") %>% 
  ggbarplot(x = "Season", y = "val", fill="Season", facet.by = "Poligono", add=c("mean_se", "jitter"))+
 facet_grid2(vars(orders), vars(Poligono), scales = "free", labeller = label_parsed, strip = strip)+  theme(axis.text.x = element_blank() ,
        axis.title.x = element_blank(),
        axis.ticks.x = element_blank())+
  ylab("Effective number of features")+
 stat_compare_means(aes(group = Season), method = "t.test",label = "p.format")+
...
ggsave('../Plots/alpha.png', width = 11, height = 11, dpi = 300, plot =alpha)
