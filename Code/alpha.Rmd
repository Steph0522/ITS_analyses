...
get_letters <- function(data, y_pos) {
  m <- aov(lm(val ~ Poligono * Season, data = data))
  p <- summary(m)[[1]][["Pr(>F)"]][1:3]

  if (p[3] < 0.05) {   # interacción significativa
    lets <- HSD.test(aov(lm(val ~ interact, data = data)), "interact")$groups %>%
      rownames_to_column("interact") %>%
      separate(interact, c("Poligono", "Season"), remove = FALSE) %>%
      mutate(xpos = as.numeric(factor(Season, levels = c("Dry", "Rainy"))))
  } else {             # solo polígono
    lets <- HSD.test(aov(lm(val ~ Poligono, data = data)), "Poligono")$groups %>%
      rownames_to_column("Poligono") %>%
      mutate(xpos = 1.5)
  }
  mutate(lets, val = y_pos)
}

lab0 <- get_letters(lm.q0, 1600)
lab1 <- get_letters(lm.q1, 250)
lab2 <- get_letters(lm.q2, 80)
...
fmt_p <- function(p) ifelse(p < 0.001, "<0.001", format(round(p, 3), nsmall = 3))
...
geom_label(data = lab0, aes(x = xpos, y = val, label = groups), 
           inherit.aes = FALSE)+
ggtitle(paste0("Polygon = ", fmt_p(pvalues0[1]), "; ",
               "Season = ", fmt_p(pvalues0[2]), "; ",
               "Polygon*Season = ", fmt_p(pvalues0[3])))+
...
# stat_compare_means(aes(group = Season), method = "t.test",label = "p.format")+
  scale_fill_manual(values = c("#A16928" ,"#2887a1"))+theme_grey()+
     theme(axis.text.x = element_blank(), 
        ...
        strip.text.y = element_text(size = 16, face="bold.italic", colour = "white"),
       axis.title = element_text(size = 18))+
geom_label(data = lab1, aes(x = xpos, y = val, label = groups), 
           inherit.aes = FALSE)+ggtitle(paste0("Polygon = ", fmt_p(pvalues1[1]), "; ",
               "Season = ", fmt_p(pvalues1[2]), "; ",
               "Polygon*Season = ", fmt_p(pvalues1[3])))+ theme(
     ...
     theme(axis.text.x = element_text(size = 14, colour = "black"), 
      #  axis.ticks.x = element_blank(),
       strip.text.x = element_text(size = 14, face = "bold", color="white"),
        strip.text.y = element_text(size = 16, face="bold.italic", colour = "white"),
       axis.title = element_text(size = 18))+
geom_label(data = lab2, aes(x = xpos, y = val, label = groups), 
           inherit.aes = FALSE)+
ggtitle(paste0("Polygon = ", fmt_p(pvalues2[1]), "; ",
               "Season = ", fmt_p(pvalues2[2]), "; ",
               "Polygon*Season = ", fmt_p(pvalues2[3])))+ theme(
                  strip.background.y = element_rect(
     color="grey", fill="black", size=1, linetype="solid"),
     strip.background.x = element_rect(colour = "black", linetype = "solid", size = 1))+
  theme(strip.text.x = element_blank())
